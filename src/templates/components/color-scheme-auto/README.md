...
It declares a base `color-scheme: normal` and then tightens to `only light` or `only dark` via the media queries defined in `styles.css`, so the entire theme follows the system preference without forcing a fixed theme.
