...
> `howiehz-higan-zh-hans-tiny.zip` 为简体中文轻量安装包，仅保留 `.woff2` 默认字体与 `.avif` 默认 logo，且不包含预压缩文件。
...
> `howiehz-higan-en-tiny.zip` is the English tiny package that keeps only the default `.woff2` fonts and `.avif` default logo, without precompressed assets.
