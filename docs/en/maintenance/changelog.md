...
- Added an optional post edit button that redirects after sign-in and is visible to authenticated users only with post management permission.
...
- Fixed the page language priority so explicit `?language=` query parameters take precedence over page metadata.
...
- Changed the default page language and multilingual content language defaults from `zh` to `zh-CN`, aligning with the Halo CMS default.
  **If the previous value was `zh`, change `Global -> Default Page Language` to `zh-CN` after updating.**
- Renamed Chinese i18n language files from `zh_CN.properties` / `zh_TW.properties` to `zh_Hans.properties` / `zh_Hant.properties`.

### 🚀 Features

- Added a "Sync Cookie language based on page content language" option to control whether the `language` Cookie is synchronized based on the page content language.
- When "Sync Cookie language based on page content language" is enabled, the theme keeps the `language` cookie aligned with the actual content language as much
  as possible to avoid inconsistent content languages in subsequent requests. This option is enabled by default.
- Enhanced candidate matching for common Chinese browser language codes in exact-match mode for auto redirect based on browser language.
...
- Fixed the "Post page style -> Post title uppercase" setting not taking effect.
...
- Added an option to control the mobile footer navigation Home button on post and single pages.
...
- Fixed hidden content in top and bottom navigation on post and single pages remaining briefly reachable by screen readers or keyboard focus while collapsing.
...
- Fixed a visual inconsistency in the RSS icons on the home, author, category, and tag pages.
- Fixed an issue where the light/dark mode toggle controller script was not injected together with the toggle button component, causing all three icons to always be loaded.
...
- Refined nightly prerelease trigger checks to count only non-merge commits within
  ...
  and after the latest stable tag,
  while excluding commit subjects starting with `docs`.
