...
- Adjusted stable and nightly version derivation to use commit-message semantics between the latest stable tag and the latest commit, picking the highest level for MAJOR / MINOR / PATCH, while keeping the nightly `-alpha.yyyyMMddHHmmssSSS` suffix.
...
- Refined nightly prerelease trigger checks to count only non-merge commits within the previous Asia/Shanghai day window and after the latest stable tag, while excluding commit subjects starting with `docs`.
...
[Unreleased]: https://github.com/HowieHz/halo-theme-higan-hz/compare/v1.66.6...HEAD
