...
- Added ["Heading anchor SVG icon"](/en/guide/theme-configuration#heading-anchor-svg-icon) setting (Post Page Style -> Heading anchor SVG icon) to use a custom `<svg>` element as the heading anchor icon. When configured, it takes precedence over the anchor symbol setting. The icon color automatically follows the theme's primary color.
