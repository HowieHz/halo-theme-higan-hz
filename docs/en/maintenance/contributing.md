...
6. After verification passes, two publishing actions run in parallel:
   ...
   - Sync to the Halo App Store by creating a draft release first,
     ...
     while ensuring `howiehz-higan-zh-hans.zip` stays first in the
     release asset list so Halo CMS can prefer the Simplified Chinese
     package during automatic updates.
