...
启用[浏览器语言自动跳转](#浏览器语言自动跳转)后，若浏览器语言与默认页面语言不同，且浏览器语言存在于此项，将自动跳转到对应页面。
...
- [表格每行底部的表格线宽度（除表头）](#表格每行底部的表格线宽度-除表头)
...
（[总体样式 -> 表格每行底部的表格线（除表头）](#表格每行底部的表格线-除表头)启用时显示）

总体样式 -> 表格每行底部的表格线宽度（除表头）

:::
::: info ⚡ 快速跳转

<QuickJumpConfig to="/console/theme/settings/styles#:~:text=表格每行底部的表格线宽度" />
