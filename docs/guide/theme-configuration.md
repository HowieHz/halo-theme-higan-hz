...
置空则使用默认值 `#`。支持任意字符串。该配置项通过注入 `--heading-anchor-symbol` CSS 变量实现。配置了下方"标题锚点 SVG 图标"后此设置将被覆盖。
...
使用自定义 SVG 图标作为文章页及独立页面中的标题锚点，优先于"标题锚点符号"设置。图标颜色自动跟随主题色。
...
<QuickJumpConfig to="/console/theme/settings/post_styles#:~:text=标题锚点 SVG 图标" />
...
- 每个自定义按钮需配置：识别码、名称、链接、图标（必填）、`aria-label`（无障碍标签）
