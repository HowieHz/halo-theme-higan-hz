...
- 修复未安装[额外 API](https://www.halo.run/store/apps/app-di1jh8gd) 插件时，多元文章列表因无法读取字数统计所需的正文而导致首页渲染失败的问题。
...
- 最低 Halo 版本要求提升至 2.25.0，以支持多个 `<meta name="theme-color">` 标签。

### 🚀 新功能

- 优化浏览器地址栏主题色（`<meta name="theme-color">`），支持在浅色/深色模式下使用对应颜色，并在主题切换时同步更新。

### 🔧 代码重构

- 移除深浅色模式切换按钮脚本中未使用的自定义配色配置查询。

### 📄 文档

- 在插件兼容性文档中补充图库页推荐搭配图片灯箱插件使用。
...
- 使用 Oxc 压缩构建输出中的传统内联脚本，跳过模块脚本、脚本数据与 Thymeleaf JavaScript 内联模板，以优化 HTML 文档体积。
...
- 修正浏览器按语言自动跳转的中文候选规则，使其与 Halo CMS 后端语言资源查找逻辑保持一致。

### 📄 文档

- 修正多语言教程与配置表单中关于语言匹配依据、默认项匹配规则与 Cookie 语言同步规则的说明。
...
- 修复页面语言设定优先级与用户显式指定语言不一致的问题，使 `?language=` 优先于页面元数据。
- 修复页面元数据语言与 `?language=` 查询参数不一致时，语言 Cookie 同步可能导致页面反复刷新的问题。
...
- 默认页面语言和多语言内容语言默认值由 `zh` 改为 `zh-CN`，与 Halo CMS 默认值一致。**如果原先是 `zh`，版本更新后请将“全局 -> 默认页面语言”改为 `zh-CN`。**
- 中文 i18n 语言文件由 `zh_CN.properties` / `zh_TW.properties` 重命名为 `zh_Hans.properties` / `zh_Hant.properties`。

### 🚀 新功能

- 新增“是否根据页面内容语言同步 Cookie 语言”配置项，可控制是否根据页面内容语言同步 `language` Cookie。
  - 开启“是否根据页面内容语言同步 Cookie 语言”后，主题会尽可能让 `language` cookie 与实际内容语言同步，避免后续请求出现内容语言不一致的情况。此项默认开启。
- 增强浏览器按语言自动跳转在精确匹配模式下对常见中文浏览器语言代码的候选匹配。
...
- 重命名构建产物中的静态资源，使内容未变更的资源在不同版本间保持稳定文件名，提升浏览器缓存复用效率。
...
- 移除自定义配色教程中过时的 Halo CMS 渲染 BUG 提示和截图。
...
- 修复“文章页样式 -> 文章标题大写”不生效的问题。
- 修复瞬间页视频弹窗打开和关闭过渡动画不生效的问题。
...
- 重构 Mermaid 图表来源收集与渲染流程。

### 🐛 错误修复

- 修复带 YAML frontmatter 的 Mermaid 图表可能无法正常渲染的问题。
...
- 新增文章页和独立页面移动端底部导航首页按钮开关。

### 🔧 代码重构

- 优化平板端文章页和独立页面目录浮层的滚动区域，使浮层标题、关闭按钮和分隔线保持固定，仅目录内容滚动。

### 🐛 错误修复

- 修复文章页和独立页面在桌面端窄宽度、移动端宽宽度，以及平板端较窄或较宽等视口与设备类型不匹配时目录不生成的问题。
- 修复文章页和独立页面顶部与底部导航收起时，隐藏内容仍可能短暂被读屏或键盘焦点访问的问题。
...
- 修复桌面端和平板端文章页顶部分享列表展开后，文章标题和文章元数据向左避让量过大的问题。
- 修复文章页元数据中发布时间和更新时间可能被拆成两行的问题。
- 稳定文章页、独立页面和首页多元列表中文章元数据项之间，以及分类、标签、阅读量、预计阅读时长与字数统计等元数据项内部图标、文本和分隔符的间距为 1ch，并统一断行位置与垂直方向对齐。
- 修复带子菜单的菜单项中文字在垂直方向不对齐的问题。
- 修复桌面端文章页目录的默认左内边距导致文章标题和文章元数据向左避让量过大的问题。
- 优化桌面端和平板端文章页顶部菜单触发避让收缩和恢复时的过渡，减少文章标题和文章元数据在 hover 操作按钮时的抖动。
- 优化桌面端和平板端文章页顶部操作按钮的 hover 文案淡入淡出显示，避免快速掠过按钮时多个文案同时显示。
- 修复 CSS 动画快速切换时旧 `animationend` 回调可能影响新动画状态的问题。
...
- 修复文章页导航菜单图标高度不居中的问题。
- 修复组件样式先于全局样式加载时 CSS cascade layer 顺序错误，导致基础样式可能覆盖 Tailwind 工具类的问题。
- 修复子菜单切换按钮始终显示向下箭头的问题。
- 修复桌面端和平板端文章页顶部菜单可能与文章标题和文章元数据重叠的问题。
- 修复平板端文章页目录浮层多个入口按钮的 `aria-expanded` 状态不同步的问题。
- 修复移动端文章页底部导航初始化时未使用当前滚动位置，导致浏览器恢复滚动位置后首次滚动方向判断不正确的问题。
- 修复移动端文章页恢复滚动位置后底部回到顶部按钮初始显隐状态不正确的问题。

### 🔧 代码重构

- 将移动端页头标题垂直居中实现由 table-cell 布局改为 Flexbox。
- 优化文章页底部菜单项包含子菜单时的切换按钮位置。
...
- 优化深浅色模式切换按钮大小，使其与站点标题区域的视觉比例更协调。
...
- 修复深浅色模式切换按钮的控制脚本未跟随按钮组件注入，导致三个图标始终加载的问题。
...
- 将二维码分享页的前端二维码生成库从 `qrious` 迁移为 `EasyQRCodeJS`。
...
- 重构 Tailwind CSS 样式产出方式，按组件拆分并复用样式资源，以提高资源利用率。
- 将 Tailwind CSS 样式压缩前缀由 `tw-` 调整为 `_`。

### 🐛 错误修复

- 修复 tiny 版本中，因 Mermaid / instant.page 组件模板路径切换不正确而导致页面 404 的问题。
...
- 更新依赖。
- 让 `default` 和 `full` 主题包共享同一次 `full` 构建产物，仅在打包阶段分流完整预压缩版与默认版。
...
- 调整 nightly 预发布触发检查：仅统计前一个自然日（Asia/Shanghai）且位于上一个正式版 Tag 之后的非 `docs` 前缀提交，并排除 merge commit。
