...
- 新增"标题锚点符号"配置项（文章页样式 -> 标题锚点符号），可自定义标题锚点链接前显示的符号，置空则使用默认值 `#`，通过注入 `--heading-anchor-symbol` CSS 变量实现。
- 新增"标题锚点符号原始输出"配置项（文章页样式 -> 标题锚点符号原始输出），开启后锚点符号值将不加引号直接作为 CSS 变量值输出，供熟悉 CSS `content` 属性语法的进阶用户使用。
- 新增"标题锚点 SVG 图标"配置项（文章页样式 -> 标题锚点 SVG 图标），支持粘贴完整 `<svg>` 标签作为标题锚点图标，配置后优先于锚点符号设置，图标颜色自动跟随主题色。
