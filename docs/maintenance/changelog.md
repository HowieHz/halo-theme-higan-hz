...
- 调整正式版与测试版的版本号推导规则：改为基于上一个正式版 Tag 到最新提交之间的提交标题语义，按最高级别决定 MAJOR / MINOR / PATCH，再保留 nightly 的 `-alpha.yyyyMMddHHmmssSSS` 后缀。
- 同步更新发布守卫与发版工作流，确保正式版和测试版都按同一套提交语义规则出号。
- 为带有 `release` 标签的 PR 增加 `package.json` 版本号自动修正，并要求发布 PR 的 base 必须是目标分支最新提交。
