...
本项目的主题发布分为正式版和测试版两类，二者都会先完成构建产物签名校验，再发布全部主题构建产物；其中正式版会同步到 Halo 应用市场，而测试版是否同步取决于触发方式。
...
在正式版与测试版工作流中，`gh attestation verify` 会作为发布门禁先校验全部 `.zip` 产物；门禁通过后，GitHub Release 与 Halo 应用市场同步（如启用）会并行进行。
