...
主题设置被重新划分为「全局」「总体样式」「首页样式」「文章页样式」「分类集合页样式」「分类详情页样式」「标签集合页样式」「标签详情页样式」「作者详情页样式」「归档页样式」「自定义页面样式」「相册页样式」「瞬间页样式」「社交资料 / RSS」「自定义分享按钮」「错误页样式」等分组。以下概览列举每个分组的新增能力（部分摘录，完整字段说明请参阅《[配置总览](../guide/configuration-overview.md)》）：
...
更多插件使用说明详见《[进阶功能](../guide/advanced.md)#可选插件》与《[内容增强与多语言](../guide/content-components.md)#mermaid-与编辑器写法》。
